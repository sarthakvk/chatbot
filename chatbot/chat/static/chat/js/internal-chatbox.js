...
function add_message_to_chat(data, formatted_div){
	...
	var new_source = data['message']["source"];
	if (new_source == "BOT"){
...
function sendTextMessage(inp) {

    message = {}
    message.text = inp;
    message.command= 'send'
    message.timestamp = new Date();
    message.source = "CANDIDATE"
    message.type = "text"
    
    var formatted_div = generate_formatted_chat_message(message)
    var data = {"message": message};

    if(formatted_div.length > 0) {
        add_message_to_chat(data, formatted_div)
    }
